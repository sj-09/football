...
utils.mixin = function (to, from){
  utils.forEach(from, function(val, key){
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (utils.isArray(val) || utils.isObject(val)) {
<span class="cstat-no" title="statement not covered" >      to[key] = utils.mixin(val, to[key] || {});</span>
    } else {
      to[key] = val;
    }
  });
